...
%----------------------------------------------------------------------------------------------%
base_path     = 'C:\Users\Dan\Moorcroft_Lab\Data\Harvard Forest Archive\';
...
msk    = ~(data(:,5) == -9999);                 % Set mask to ignore -9999 i.e. missing vals.
data(msk,5:6) = data(msk,5:6) *fact_1 *fact_2;  % ... and convert NEE, umol/m2/s -> tC/ha

msk    = ~(data(:,9) == -9999);                 % Set mask to ignore -9999 i.e. missing vals.
data(msk,9:10) = data(msk,9:10)*fact_1 *fact_2; % ... and convert NEE,  -> permil*tC/ha
...
data        = [data, NaN(length(data),4)];      % Append columns for daily, nightly NEE. 

dmeans = NaN(nyrs*366,7);                       % Initialize daily   means matrix
mmeans = NaN(nyrs*12 ,6);                       % Initialize monthly means matrix
ymeans = NaN(nyrs    ,5);                       % Initialize yearly  means matrix
...
for yr_num = 1:nyrs
   ...
   yr_dat(yr_dat == -9999) = NaN;
   ymean  = nansum([yr_dat(:,5), yr_dat(:,9)]);    % Get total yearly flux
   
   if yr < 2005
      runc = unc(yr_num)/ymean(1);                 % Get relative uncertainty for year.
      yunc = [unc(yr_num) abs(runc*ymean(2))];     % Get yearly uncertainty; If we have info use
   else                                            % it, otherwise use the average proportional
      yunc = abs(0.2*ymean);                       % uncertainty multiplied by the yearly NEE.
   end
   
   ymeans(yr_num,:) = [yr, ymean(1), yunc(1), ymean(2), yunc(2) ];
   ymeans(ymeans == 0) = -9999;                    % Turn zeros (from nansum) back into NaNs
   ...
   for imo = yr_dat(1,2):yr_dat(end,2)
      ...
      mo_dat(mo_dat == -9999) = NaN;               % Convert missing vals to NaNs
      
      mmean          = nansum([mo_dat(:,5) ...     % Sum to get total monthly flux
                               mo_dat(:,9)]);
      
      num_data = sum(~isnan(mo_dat(:,5)));
      num_poss = numel(mo_dat(:,5));
      dat_frac = num_data/num_poss;

      munc           = yunc/12*dat_frac;           % Assume all months contr. = to yrly unc
      mmeans(mcnt,:) = [yr, imo, mmean(1),...      % Save to the pre-allocated matrix.
                                 munc(1),... 
                                 mmean(2),...
                                 munc(2)];
      mmeans(mmeans == 0) = -9999;                 % Turn zeros (from nansum) back into NaNs
      ...
      for id = mo_dat(1,3):mo_dat(end,3)           % The 'd' in 'id' is actually day of year
         ...
         d_dat(d_dat == -9999) = NaN;
         
         dmean          = nansum([d_dat(:,5) d_dat(:,9)]);  % Sum to get total daily flux
         dunc           = munc/mo_days(imo);       % Assume all days contr. = to monthly unc
         
         dmeans(dcnt,:) = [yr,imo,dom,dmean(1),... % Save to the pre-allocated matrix.
                                      dunc(1),...
                                      dmean(2),...
                                      dunc(2)];
         dmeans(dmeans == 0) = -9999;              % Turn zeros (from nansum) back into NaNs
         ...
         hr_dat = data(ind1:ind2,[5:6,11:14]);     % Extract hourly NEE from day for processing.
         ...
         data(ind1:ind2,11:12) = hr_dat(:,3:4);    % Xfer NEE_Night back to 'data'
         data(ind1:ind2,13:14) = hr_dat(:,5:6);    % Xfer NEE_Day back to 'data'
...
header = ['Year, Month, Day, Hour, NEE, NEE_sd, Latent, Latent_sd, NEE_Iso, NEE_Iso_sd, '...
          'NEE_Night, NEE_Night_sd, NEE_Day, NEE_Day_sd\n'];
fprintf(fid,header);
