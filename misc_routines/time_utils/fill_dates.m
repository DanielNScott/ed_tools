function [ times ] = fill_dates(res,start,fin,inc)
...
% Set prefix and suffix for filenames, initialize 'times'
times = {};
...
while ( yr < end_yr || mo < end_mo || d < end_d || hr < end_hr || min < end_min || sec < end_sec )
   ...
   if iter > maxiter;
      ...
      disp(' Or > 20 years of instantaneous fluxes...?')
   ...
   if sum(strcmp(res,{'Y','M','D','I'}))
      ...
      times{end+1} = [yrstr '-' mostr '-' dstr '-' hrstr '-' minstr '-' secstr] ;
