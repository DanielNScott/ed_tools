function [ ] = plot_opt_figs_DMR_ISO( opt_mat_name, save)
...
%      freezeColors();
   ...
   elseif i == 2;
      hold on
      ciplot(envel{2}{1}(8:end),envel{2}{2}(8:end),1:24,'r');
      plot(1:24,yvals{2})
      hold off
%      freezeColors();
   elseif i == 3;
      npts = numel(hist.out_first.Y.DMEAN_NEE);
      hold on
      ciplot(envel{3}{1}(215:end),envel{3}{2}(215:end),1:npts,'r')
      plot(1:npts,yvals{3})
      hold off
%      freezeColors();
   elseif i == 4;
...
yvals{1} = [hist.out_first.Y.YMEAN_NEE_ISOFLX', hist.out_best.Y.YMEAN_NEE_ISOFLX', pntr{1}.NEE];
yvals{2} = [hist.out_first.Y.MMEAN_NEE_ISOFLX'; hist.out_best.Y.MMEAN_NEE_ISOFLX'];
yvals{3} = [hist.out_first.Y.DMEAN_NEE_ISOFLX', hist.out_best.Y.DMEAN_NEE_ISOFLX'];
yvals{4} = [-1* hist.stats.likely.yearly.NEE_Iso(:,1), ...
            -1* hist.stats.likely.yearly.NEE_Iso(:,hist.iter_best)];
yvals{5} = [-1* hist.stats.likely.monthly.NEE_Iso(:,1); ...
            -1* hist.stats.likely.monthly.NEE_Iso(:,hist.iter_best)];
yvals{6} = [-1* hist.stats.likely.daily.NEE_Iso(:,1)';...
            -1* hist.stats.likely.daily.NEE_Iso(:,hist.iter_best)'];
         
zero_pad = zeros(ndyr,1);
envel{1} = [zero_pad,zero_pad,2*pntr{1}.NEE_Iso_sd];
envel{2} = {pntr{2}.NEE + 2*pntr{2}.NEE_Iso_sd, pntr{2}.NEE_Iso - 2*pntr{2}.NEE_Iso_sd};
envel{3} = {pntr{3}.NEE - 2*pntr{3}.NEE_Iso_sd, pntr{3}.NEE_Iso + 2*pntr{3}.NEE_Iso_sd};
...
titles  = {'Yearly Mean NEE Isoflux'   , 'Monthly Mean NEE Isoflux'   , 'Daily Mean NEE Isoflux',...
           'Yearly Likelihoods', 'Monthly Likelihoods', 'Daily Likelihoods'};
xlabs   = {'Years' , 'Months', 'Days since Jan 1, 1994' , ...
           'Years' , 'Months', 'Days since Jan 1, 1994'};
ylabs   = {'tC/ha', 'tC/ha', 'tC/ha', ...
...
% err{1} = pntr.BAG_sd;
% err{2} = pntr.BAM_sd;
% err{3} = pntr.BAG_Hw_sd;
% err{4} = [];
% err{5} = [];
% err{6} = [];
% err{7} = pntr.BAM_Hw_sd;
% err{8} = pntr.BAG_Co_sd;
% err{9} = pntr.BAM_Co_sd;
%          
% lgnds{1} = {'Initial';'Best';'Obs.'};
