...
if sum(strcmp(model,{'ED2.1','out.mat','read_dir'}));
   obs = preproc_obs(obs, out, opt_metadata);
...
%   Detailed explanation goes here
   ...
   for res_num = 1:numel(resolutions)                       % Cycle through the resolutions
      ...
      for fld_num = 1:numel(fields)                         % Cycle through fields
         ...
         mdr_typ_num  = strcmp(opt_metadata(:,2),fld);      % Get rows in metadata w/ this type 
         mdr_res_num  = strcmp(opt_metadata(:,1),res);      % Get rows in metadata w/ this res 
         mdr_num      = and(mdr_typ_num,mdr_res_num);       % Want row with this type and res
         ...
         if ~isempty(metadata_row)                          % It's empty if no metadata exists
            out_fld  = metadata_row{4};                     % Data's field name in output
            ...
            problem_flds = ...
               {'BAG', 'BAG_Hw', 'BAG_Co', ...
                'BAM', 'BAM_Hw', 'BAM_Co', ...
                'BAR', 'BAR_Co', 'BAR_Co', };
            if sum(strcmp(fld,problem_flds));
               obs_data = obs_data(2:end);                  % Some issue... !
