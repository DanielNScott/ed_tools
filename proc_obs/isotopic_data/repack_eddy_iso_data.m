...
NEE_d13C    = NEE_Iso ./ NEE;
NEE_d13C_sd = 0.21495 + 14.204* abs(NEE).^(-0.95502);
...
hr_data = [dates, NEE, NEE_Unc, NEE_Iso, NEE_Iso_Unc, NEE_d13C, NEE_d13C_sd, ...
           meanDel13C, d13C_Unc, meanCO2, CO2_Unc];
...
% Export Data                                                                                  %
%----------------------------------------------------------------------------------------------%
header_line_1 = ['"# NEE is in [umol/m2/s], NEE_Iso is in [umol/m2/s*permilVPDB], '... 
                 'meanDel13C is in [permilVPDB], meanCO2 is ppm"\n'];

header_line_2 = ['Year, Month, Day, Hour, NEE, NEE_sd, NEE_Iso, NEE_Iso_sd, NEE_d13C, '...
                 'NEE_d13C_sd, meanDel13C, d13C_Unc, meanCO2, CO2_Unc \n'];
              
fid = fopen('eddy_isoflux.csv','wt');
fprintf(fid,header_line_1);
fprintf(fid,header_line_2);
dlmwrite('eddy_isoflux.csv',hr_data,'delimiter',',','-append');
fclose(fid);
%----------------------------------------------------------------------------------------------%
end